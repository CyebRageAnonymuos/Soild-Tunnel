...
use crate::soildnoize::{self, SoildNoizeConfig};
...
pub struct WgConfig {
    ...
    pub soildnoize: Arc<SoildNoizeConfig>,
}
...
impl WgTunnel {
    pub async fn new(cfg: WgConfig, inbound_tx: mpsc::Sender<Vec<u8>>) -> Result<Self> {
        ...
        let tunn = Tunn::new(local_secret, peer_public, preshared, cfg.persistent_keepalive, 0, None)
            .map_err(|e| SoildTunnelError::Other(format!("wireguard tunnel init: {e}")))?;
        ...
        Ok(Self {
            ...
            soildnoize: cfg.soildnoize.clone(),
    ...
    pub fn from_established(
        ...
        soildnoize: Arc<SoildNoizeConfig>,
        inbound_tx: mpsc::Sender<Vec<u8>>,
        local_ipv4: Ipv4Addr,
    ) -> Self {
        Self {
    ...
    pub async fn run(self, mut outbound_rx: mpsc::Receiver<Vec<u8>>) -> Result<()> {
        ...
        let soildnoize = self.soildnoize.clone();
        let soildnoize_t = self.soildnoize.clone();
                ...
                match tunn.encapsulate(&ip_packet, &mut out_buf) {
                    ...
                    TunnResult::WriteToNetwork(pkt) => {
...

                        {
                            ...
                            if !*sent && soildnoize.is_enabled() {
                                *sent = true;
                                drop(sent);
                                soildnoize::apply_obfuscation(&sock_w, peer, &soildnoize).await;
                            }
                        }
                        ...
                        if soildnoize.jc_after_hs > 0 && !post_hs_junk_sent {
                            post_hs_junk_sent = true;
                            soildnoize::send_post_handshake_junk(&sock_w, peer, &soildnoize).await;
                        }
                    }
        ...
        let timer_task = tokio::spawn(async move {
            ...
            loop {
                ...
                if let TunnResult::WriteToNetwork(pkt) = tunn.update_timers(&mut tmp) {
                    ...
                    if soildnoize_t.is_enabled() {
                        soildnoize::send_keepalive_junk(&sock_t, &soildnoize_t).await;
                    }
        ...
        let health_task = tokio::spawn(async move {
            ...
            loop {
                ...
                if idle >= stale_timeout {
                    ...
                    return Err::<(), SoildTunnelError>(SoildTunnelError::Other(
        ...
        let result = tokio::select! {
            ...
            r = health_task => {
                match r {
                    Ok(Err(e)) => Err(e),
                    Ok(Ok(())) => Ok(()),
                    Err(e) => Err(SoildTunnelError::Other(format!("health task panicked: {e}"))),
                }
            }
        };
...
fn wg_stale_timeout() -> Duration {
    let secs = std::env::var("SOILDTUNNEL_WG_STALE_SECS")
        .ok()
...
async fn send_dataplane_probe(
    ...
    match tunn.encapsulate(probe, out_buf) {
        ...
        TunnResult::Err(e) => {
            return Err(SoildTunnelError::Other(format!("dataplane encap: {e:?}")));
        }
        _ => {}
    }
    Ok(())
...
async fn verify_dataplane(
    ...
    loop {
        ...
        if now >= deadline {
            ...
            return Err(SoildTunnelError::Other("dataplane timeout".into()));
        }
        if now >= resend_at {
...
pub async fn verify_endpoint_keep_session(
    ...
    let data_check = std::env::var("SOILDTUNNEL_WG_NO_DATA_CHECK").is_err();
    log::trace!("[wg] verify {} obf={} data_check={}", peer, soildnoize.is_enabled(), data_check);
    ...
    if soildnoize.is_enabled() {
        soildnoize::apply_obfuscation(&sock, peer, soildnoize).await;
    }
    ...
    let mut tunn = Tunn::new(local_secret, peer_pk, None, Some(keepalive.unwrap_or(25)), 0, None)
        .map_err(|e| SoildTunnelError::Other(format!("tunn init: {e}")))?;
    ...
    let init_packet = match tunn.encapsulate(&[], &mut out_buf) {
        ...
        other => {
            ...
            return Err(SoildTunnelError::Other("handshake init failed".into()));
        }
    };
    ...
    loop {
        if Instant::now() >= deadline {
            ...
            return Err(SoildTunnelError::Other("verify timeout".into()));
        }
        ...
        tokio::select! {
            ...
            _ = timer.tick() => {
                ...
                match tunn.update_timers(&mut out_buf) {
                    ...
                    TunnResult::Err(e) => {
                        return Err(SoildTunnelError::Other(format!("wireguard timer failed: {e:?}")));
                    }
                    _ => {}
                }
            }
...
mod tests {
    ...
    async fn endpoint_verification_retransmits_a_lost_initial_handshake() {
        ...
        let profile = soildnoize::from_profile("off");
