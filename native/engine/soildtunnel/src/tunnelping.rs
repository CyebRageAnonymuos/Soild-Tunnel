...
fn http_probe_port() -> u16 {
    std::env::var("SOILDTUNNEL_IRONCLAD_PORT")
        .ok()
...
async fn http_probe(stack: &netstack::StackHandle) -> Result<()> {
    ...
    if timed_out {
        return Err(SoildTunnelError::Other("http probe response timeout".into()));
    }
    ...
    if http_status_code(status_line) == Some(204) {
        ...
        Err(SoildTunnelError::Other(format!(
...
pub async fn masque_http_ping(p: &MasquePingParams, timeout: Duration) -> Result<Duration> {
    ...
    match tokio::time::timeout(timeout, attempt).await {
        Ok(Ok(rtt)) => Ok(rtt),
        Ok(Err(e)) => Err(e),
        Err(_) => Err(SoildTunnelError::Other("ironclad http probe timeout".into())),
    }
}
...
pub async fn wg_http_ping_established(
    ...
    let attempt = async {
        ...
        let tunnel = wireguard::WgTunnel::from_established(
            session,
            std::sync::Arc::new(p.soildnoize.clone()),
            inbound_tx,
            p.local_ipv4,
        );
