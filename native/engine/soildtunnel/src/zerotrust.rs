...
impl TeamSettings {
    pub fn from_env() -> Option<Self> {
        let team = normalize_team(&std::env::var("SOILDTUNNEL_TEAM").unwrap_or_default())?;
        ...
        Some(Self {
            team,
            client_id: non_empty("SOILDTUNNEL_ACCESS_CLIENT_ID"),
            client_secret: non_empty("SOILDTUNNEL_ACCESS_CLIENT_SECRET"),
            token: non_empty("SOILDTUNNEL_ACCESS_TOKEN"),
            email: non_empty("SOILDTUNNEL_ACCESS_EMAIL"),
        })
    }
...
pub async fn store_token(token: &str) -> Result<()> {
    ...
    if !looks_like_jwt(token) {
        return Err(SoildTunnelError::Api(
...
async fn sign_in(settings: &TeamSettings) -> Result<String> {
    ...
    Err(SoildTunnelError::Api(format!(
        "team {} needs a way to sign in. pick one: a service token via \
         SOILDTUNNEL_ACCESS_CLIENT_ID and SOILDTUNNEL_ACCESS_CLIENT_SECRET, an email one-time code via \
         SOILDTUNNEL_ACCESS_EMAIL, or a token you already hold via SOILDTUNNEL_ACCESS_TOKEN \
         (sign in at {})",
        settings.team,
        settings.login_url()
    )))
...
fn access_client() -> Result<reqwest::Client> {
    reqwest::Client::builder()
        ...
        .cookie_store(true)
        .build()
        .map_err(|e| SoildTunnelError::Api(format!("access client: {e}")))
}
...
impl EmailSignIn {
    ...
    pub async fn submit_code(&self, code: &str) -> Result<CodeOutcome> {
        let code = code.trim();
        if code.is_empty() {
            return Err(SoildTunnelError::Api("no login code was entered".into()));
        }
        ...
        let confirmed = self
            ...
            .send()
            .await
            .map_err(|e| SoildTunnelError::Api(format!("confirming the login code: {e}")))?;

        let status = confirmed.status();
        let body = confirmed
            .text()
            .await
            .map_err(|e| SoildTunnelError::Api(format!("callback body: {e}")))?;
...
pub async fn begin_email_signin(settings: &TeamSettings, email: &str) -> Result<EmailSignIn> {
    ...
    let landing = client
        ...
        .send()
        .await
        .map_err(|e| SoildTunnelError::Api(format!("enrolment page: {e}")))?;

    let login_url = landing.url().to_string();
    let landing_body = landing
        .text()
        .await
        .map_err(|e| SoildTunnelError::Api(format!("enrolment page body: {e}")))?;

    let verify_url = extract_totp_form_action(&landing_body).ok_or_else(|| {
        SoildTunnelError::Api(format!(
...
async fn request_email_code(
    ...
    let sent = client
        ...
        .send()
        .await
        .map_err(|e| SoildTunnelError::Api(format!("requesting a login code: {e}")))?;
    ...
    if !status.is_success() {
        return Err(SoildTunnelError::Api(format!(
...
async fn prompt_login_code(email: &str, attempt: u32) -> Result<String> {
    ...
    match outcome {
        Ok(0) | Err(()) => Err(SoildTunnelError::Api(match interactive {
        ...
        Ok(_) => {
            ...
            if code.is_empty() {
                Err(SoildTunnelError::Api("no login code was entered".into()))
            } else {
                Ok(code)
            }
        }
...
async fn fetch_token_with_service_token(settings: &TeamSettings) -> Result<String> {
    let client_id = settings
        .client_id
        .as_deref()
        .ok_or_else(|| SoildTunnelError::Api("missing access client id".into()))?;
    let client_secret = settings
        .client_secret
        .as_deref()
        .ok_or_else(|| SoildTunnelError::Api("missing access client secret".into()))?;
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| SoildTunnelError::Api(format!("access request: {e}")))?;
