...
impl RuleSet {
    pub fn from_env() -> Self {
        let mut block = std::env::var("SOILDTUNNEL_ROUTE_BLOCK").unwrap_or_default();
        let mut direct = std::env::var("SOILDTUNNEL_ROUTE_DIRECT").unwrap_or_default();

        if let Ok(path) = std::env::var("SOILDTUNNEL_ROUTES_FILE") {
