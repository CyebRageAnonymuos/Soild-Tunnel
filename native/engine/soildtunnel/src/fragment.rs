...
impl FragmentConfig {
    ...
    pub fn from_env() -> Self {
        let enabled = std::env::var("SOILDTUNNEL_MASQUE_H2_FRAGMENT")
            .map(|v| is_truthy(&v))
            .unwrap_or(false);

        let (size_min, size_max) = parse_range(
            &std::env::var("SOILDTUNNEL_MASQUE_H2_FRAGMENT_SIZE").unwrap_or_default(),
            (16, 32),
        );
        let (delay_min_ms, delay_max_ms) = parse_range(
            &std::env::var("SOILDTUNNEL_MASQUE_H2_FRAGMENT_DELAY").unwrap_or_default(),
            (2, 10),
        );
