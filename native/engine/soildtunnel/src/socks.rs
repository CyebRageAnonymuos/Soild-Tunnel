...
use crate::error::{SoildTunnelError, Result};
use crate::netstack::{StackHandle, TcpSender, UdpSender};
use crate::routing::{Action, Host, RuleSet};

/// Kernel socket buffers for hot-path TCP relays. The long-haul tunnel leg has
/// a high bandwidth-delay product; the OS-default ~208 KiB window throttles a
/// single flow well below the link rate. Fat windows let one connection fill
/// the pipe even at 300+ ms RTT.
const SOCKET_BUFFER_BYTES: usize = 4 << 20;

/// Userspace relay chunk. Bigger reads/writes mean fewer syscalls per byte and
/// fewer wake-ups; 64 KiB matches the TUN-side readers.
const RELAY_BUFFER_BYTES: usize = 64 << 10;

/// Throughput tuning for every TCP socket on the data path: fat kernel
/// buffers + no Nagle so ACK-clocked writes are never coalesced into delays.
fn tune_tcp(stream: &TcpStream) {
    let r = SockRef::from(stream);
    let _ = r.set_recv_buffer_size(SOCKET_BUFFER_BYTES);
    let _ = r.set_send_buffer_size(SOCKET_BUFFER_BYTES);
    let _ = stream.set_nodelay(true);
}

/// Bidirectional userspace pump with an oversized chunk buffer.
/// Replaces `tokio::io::copy(_bidirectional)` (8 KiB chunks) on the hot path.
async fn copy_pipe<R, W>(rd: &mut R, wr: &mut W) -> std::io::Result<u64>
where
...
async fn handle_client(mut sock: TcpStream, stack: StackHandle, bind_ip: IpAddr) -> Result<()> {
    ...
    if head[0] != VER {
        return Err(SoildTunnelError::Other("bad socks version".into()));
    }
    ...
    match cmd {
        ...
        _ => {
            ...
            Err(SoildTunnelError::Other("unsupported socks command".into()))
        }
    }
}
...
async fn handshake(sock: &mut TcpStream) -> Result<()> {
    ...
    if prefix[0] != VER {
        return Err(SoildTunnelError::Other("bad greeting version".into()));
    }
...
async fn read_target(sock: &mut TcpStream, atyp: u8) -> Result<(Target, u16)> {
    let target = match atyp {
        ...
        _ => return Err(SoildTunnelError::Other("bad atyp".into())),
    };
...
pub(crate) fn resolver_addresses() -> Vec<SocketAddr> {
    let configured = std::env::var("SOILDTUNNEL_DNS").unwrap_or_default();
...
async fn dns_exchange(
    ...
    let mut last = SoildTunnelError::Other("dns timeout".into());
    ...
    for server in resolver_addresses() {
        ...
        loop {
            let resp = match tokio::time::timeout_at(deadline, from_stack.recv()).await {
                Ok(Some(resp)) => resp,
                Ok(None) => return Err(SoildTunnelError::Other("dns channel closed".into())),
                Err(_) => {
                    last = SoildTunnelError::Other(format!("dns timeout from {server}"));
                    break;
                }
            };
            ...
            return Err(SoildTunnelError::Other(format!("no A record for {name}")));
        }
    }
...
fn sniff_enabled() -> bool {
    !matches!(
        std::env::var("SOILDTUNNEL_ROUTE_SNIFF").as_deref(),
        Ok("0") | Ok("off") | Ok("false")
    )
}

fn sniff_window() -> Duration {
    let ms = std::env::var("SOILDTUNNEL_ROUTE_SNIFF_MS")
        .ok()
...
async fn handle_direct(
    ...
    let up = tokio::spawn(async move { copy_pipe(&mut client_rd, &mut remote_wr).await });
    let _ = copy_pipe(&mut remote_rd, &mut client_wr).await;
...
async fn open_through_gateway(
    ...
    let conn = tokio::time::timeout(GATEWAY_PROBE_TIMEOUT, stack.open_tcp(proxy))
        .await
        .map_err(|_| SoildTunnelError::Other(format!("gateway {proxy} did not accept a connection")))??;
    ...
    loop {
        let chunk = tokio::time::timeout(GATEWAY_PROBE_TIMEOUT, from_stack.recv())
            .await
            .map_err(|_| SoildTunnelError::Other(format!("gateway {proxy} did not answer in time")))?
            .ok_or_else(|| SoildTunnelError::Other(format!("gateway {proxy} closed the connection")))?;

        head.extend_from_slice(&chunk);

        if let Some(at) = find_head_end(&head) {
            let accepted = proxy_connect_succeeded(&head).ok_or_else(|| {
                SoildTunnelError::Other(format!("gateway {proxy} sent a malformed response"))
            })?;
...
async fn read_head(sock: &mut TcpStream) -> Result<Vec<u8>> {
    ...
    loop {
        ...
        if head.len() > HTTP_HEAD_LIMIT {
            return Err(SoildTunnelError::Other("http request head too large".into()));
        }
    }
}
...
async fn relay_http_direct(
    ...
    tune_tcp(&sock);
    let (mut a_rd, mut a_wr) = sock.into_split();
    let (mut b_rd, mut b_wr) = upstream.into_split();

    // a_wr is owned by the reverse pump below; dropping the halves on task end
    // is what closes the socket (no explicit shutdown possible afterwards).
    let up = tokio::spawn(async move { copy_pipe(&mut b_rd, &mut a_wr).await });
    let _ = copy_pipe(&mut a_rd, &mut b_wr).await;
    up.abort();
    Ok(())
}
...
mod sniff_route_tests {
    ...
    async fn a_server_speaks_first_protocol_survives_the_sniff_window() {
        std::env::set_var("SOILDTUNNEL_ROUTE_SNIFF_MS", "50");
        ...
        std::env::remove_var("SOILDTUNNEL_ROUTE_SNIFF_MS");
    ...
    fn sniffing_is_on_unless_it_is_turned_off() {
        std::env::remove_var("SOILDTUNNEL_ROUTE_SNIFF");
        assert!(sniff_enabled());
        std::env::set_var("SOILDTUNNEL_ROUTE_SNIFF", "0");
        assert!(!sniff_enabled());
        std::env::set_var("SOILDTUNNEL_ROUTE_SNIFF", "off");
        assert!(!sniff_enabled());
        std::env::set_var("SOILDTUNNEL_ROUTE_SNIFF", "1");
        assert!(sniff_enabled());
        std::env::remove_var("SOILDTUNNEL_ROUTE_SNIFF");
    }
}
