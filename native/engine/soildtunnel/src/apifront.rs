...
impl Fingerprint {
    ...
    fn configure(self) -> Result<boring::ssl::ConnectConfiguration> {
        ...
        let tls = |error: boring::error::ErrorStack| SoildTunnelError::Tls(error.to_string());
...
fn parse_response(raw: &[u8]) -> Result<(u16, String)> {
    ...
    let split = text
        .find("\r\n\r\n")
        .ok_or_else(|| SoildTunnelError::Api("truncated response head".into()))?;
    ...
    let status_line = lines
        .next()
        .ok_or_else(|| SoildTunnelError::Api("empty response".into()))?;
    let status = status_line
        .split_whitespace()
        .nth(1)
        .and_then(|token| token.parse::<u16>().ok())
        .ok_or_else(|| SoildTunnelError::Api(format!("bad status line: {status_line}")))?;
...
async fn exchange(
    ...
    let tcp = match crate::upstream::configured() {
        Some(proxy) => tokio::time::timeout(CONNECT_TIMEOUT, proxy.connect(address))
            .await
            .map_err(|_| SoildTunnelError::Api(format!("connect to {address} timed out")))?
            .map_err(|e| SoildTunnelError::Api(format!("connect to {address} through the proxy: {e}")))?,
        None => tokio::time::timeout(CONNECT_TIMEOUT, TcpStream::connect(address))
            .await
            .map_err(|_| SoildTunnelError::Api(format!("connect to {address} timed out")))?
            .map_err(|e| SoildTunnelError::Api(format!("connect to {address}: {e}")))?,
    };
    ...
    .await
    .map_err(|_| SoildTunnelError::Api(format!("tls handshake with {address} timed out")))?
    .map_err(|e| SoildTunnelError::Api(format!("tls handshake with {address}: {e}")))?;
    ...
    .await
    .map_err(|_| SoildTunnelError::Api(format!("exchange with {address} timed out")))?
    .map_err(|e| SoildTunnelError::Api(format!("exchange with {address}: {e}")))?;
...
pub async fn fetch(request: &ApiRequest) -> Result<ApiResponse> {
    ...
    let mut failure: Option<SoildTunnelError> = None;
    ...
    Err(failure
        .unwrap_or_else(|| SoildTunnelError::Api("every camouflaged route failed".into())))
}
