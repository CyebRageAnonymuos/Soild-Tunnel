...
use crate::{soildnoize, config, consts, dns, noize, prober, quic, wg_prober, wireguard, zerotrust};
    ...
    F: std::future::Future<Output = Result<T>>,
{
    tokio::select! {
        biased;
        _ = cancel.wait() => Err(SoildTunnelError::Cancelled),
...
impl TeamCredentials {
    pub fn new(team: &str) -> Result<Self> {
        let team = zerotrust::normalize_team(team).ok_or_else(|| {
            SoildTunnelError::Api(format!("'{team}' is not a usable zero trust team name"))
        })?;
        Ok(Self {
            team,
            ..Default::default()
...
pub struct ScanRequest {
    ...
    pub soildnoize: soildnoize::SoildNoizeConfig,
}
...
impl ScanRequest {
    pub fn for_transport(transport: Transport) -> Self {
        Self {
            ...
            soildnoize: soildnoize::from_profile("balanced"),
        }
    }
    ...
    pub fn with_profile(mut self, profile: &str) -> Self {
        ...
        self.soildnoize = soildnoize::from_profile(profile);
        self
    }
}
    ...
    match request.transport {
        ...
        Transport::WireGuard => {
            let probe = wg_prober::WgProbe {
                ...
                soildnoize: request.soildnoize.clone(),
                ports: request.ports.clone(),
                ip: request.ip,
...
fn wg_local_v4(identity: &Identity) -> Result<Ipv4Addr> {
    identity
        .ipv4
        .parse()
        .map_err(|_| SoildTunnelError::Other(format!("identity has an unusable ipv4 {}", identity.ipv4)))
}
    ...
    match spec.transport {
        ...
        Transport::WireGuard => {
            ...
            match guard(cancel, attempt).await {
                Ok(_) => Ok(true),
                Err(SoildTunnelError::Cancelled) => Err(SoildTunnelError::Cancelled),
                Err(e) => {
...
pub async fn connect(
    ...
    match spec.http {
        Some(listen) => std::env::set_var("SOILDTUNNEL_HTTP_PROXY", listen.to_string()),
        None => std::env::remove_var("SOILDTUNNEL_HTTP_PROXY"),
    }
...
mod tests {
    ...
    fn identity_paths_follow_the_same_shape_the_cli_writes() {
        assert_eq!(
            identity_path("soildtunnel.toml", Transport::Masque, None),
            "soildtunnel-masque.toml"
        );
        assert_eq!(
            identity_path("soildtunnel.toml", Transport::WireGuard, None),
            "soildtunnel.toml"
        );
        assert_eq!(
            identity_path("soildtunnel.toml", Transport::Masque, Some("acme")),
            "soildtunnel-team-acme.toml"
        );
        assert_eq!(
            identity_path("/var/lib/soildtunnel/soildtunnel.toml", Transport::WireGuard, Some("acme")),
            "/var/lib/soildtunnel/soildtunnel-team-acme.toml"
        );
    }
    ...
    async fn cancelling_stops_the_work_it_guards() {
        ...
        assert!(matches!(outcome, Err(SoildTunnelError::Cancelled)));
    }
