...
use crate::{consts, error::SoildTunnelError, error::Result};
...
async fn bind_udp_fast(bind_addr: SocketAddr) -> Result<UdpSocket> {
    ...
    let sock = Socket::new(domain, Type::DGRAM, None).map_err(SoildTunnelError::Io)?;
    sock.set_nonblocking(true).map_err(SoildTunnelError::Io)?;
    ...
    sock.bind(&bind_addr.into()).map_err(SoildTunnelError::Io)?;
    UdpSocket::from_std(sock.into()).map_err(SoildTunnelError::Io)
}
...
fn poll_h3(
    ...
    loop {
        match h3c.poll(conn) {
            ...
            Err(e) => return Err(SoildTunnelError::H3(e)),
        }
    }
...
async fn flush(
    ...
    loop {
        match conn.send(&mut out) {
            ...
            Err(quiche::Error::Done) => break,
            Err(e) => return Err(SoildTunnelError::Quic(e)),
        }
    }
...
async fn do_migrate(
    ...
    if conn.available_dcids() == 0 {
        return Err(SoildTunnelError::Other("no spare dcids for migration".into()));
    }
...
pub async fn verify_masque(p: &VerifyParams) -> Result<Duration> {
    ...
    loop {
        ...
        tokio::select! {
            r = sock.recv(&mut buf) => {
                match r {
                    ...
                    Err(e) => return Err(SoildTunnelError::Io(e)),
                }
            }
        ...
        if conn.is_closed() {
            return Err(SoildTunnelError::Other("closed before data-plane confirmation".into()));
        }
    }
}
