...
use crate::error::SoildTunnelError;
use crate::Result;
...
impl Upstream {
    pub fn from_env() -> Option<Self> {
        let raw = std::env::var("SOILDTUNNEL_UPSTREAM").ok()?;
        ...
        let kind = match scheme.as_str() {
            ...
            other => {
                return Err(SoildTunnelError::Other(format!(
                    "{other} is not an upstream proxy kind soildtunnel understands"
                )))
            }
        };
    ...
    pub fn as_reqwest_proxy(&self) -> Result<reqwest::Proxy> {
        let proxy = reqwest::Proxy::all(self.url())
            .map_err(|error| SoildTunnelError::Other(format!("{} is unusable: {error}", self.url())))?;
    ...
    pub async fn connect(&self, target: SocketAddr) -> Result<TcpStream> {
        ...
        match tokio::time::timeout(HANDSHAKE_TIMEOUT, attempt).await {
            Ok(result) => result,
            Err(_) => Err(SoildTunnelError::Other(format!(
    ...
    async fn socks_greet(&self, stream: &mut TcpStream) -> Result<()> {
        ...
        match answer[1] {
            ...
            AUTH_REJECTED => Err(SoildTunnelError::Other(
                "the upstream proxy rejected every authentication method offered".into(),
            )),
            other => Err(SoildTunnelError::Other(format!(
                "the upstream proxy asked for authentication method {other}, which soildtunnel cannot do"
            ))),
        }
    }
    ...
    async fn http_connect(&self, stream: &mut TcpStream, target: SocketAddr) -> Result<()> {
        ...
        let status = http_status(&head).ok_or_else(|| {
            SoildTunnelError::Other("the upstream proxy answer was not http".into())
        })?;
...
async fn read_reply(stream: &mut TcpStream) -> Result<SocketAddr> {
    ...
    let ip = match head[3] {
        ...
        other => {
            return Err(SoildTunnelError::Other(format!(
                "the upstream proxy sent address type {other}, which soildtunnel cannot read"
            )))
        }
    };
...
async fn relay_address(bound: SocketAddr, host: &str, port: u16) -> Result<SocketAddr> {
    ...
    tokio::net::lookup_host((host, relay_port))
        ...
        .next()
        .ok_or_else(|| {
            SoildTunnelError::Other(format!("{host} does not resolve to a usable relay address"))
        })
}
...
mod tests {
    ...
    async fn a_datagram_makes_the_round_trip_through_a_socks5_relay() {
        ...
        std::env::set_var("SOILDTUNNEL_UPSTREAM", format!("socks5://{proxy_address}"));
        ...
        std::env::remove_var("SOILDTUNNEL_UPSTREAM");
        server.abort();
    }
    ...
    async fn a_local_peer_is_reached_without_the_proxy() {
        std::env::set_var("SOILDTUNNEL_UPSTREAM", "socks5://127.0.0.1:9");
