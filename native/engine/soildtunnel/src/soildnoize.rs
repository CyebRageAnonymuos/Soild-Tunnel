...
pub fn from_profile(name: &str) -> SoildNoizeConfig {
    match name {
        "off" | "none" => SoildNoizeConfig::off(),
        "light" => SoildNoizeConfig::light(),
        "aggressive" | "heavy" => SoildNoizeConfig::aggressive(),
        _ => SoildNoizeConfig::balanced(),
    }
}
...
fn generate_junk(cfg: &SoildNoizeConfig) -> Vec<u8> {
...
pub async fn apply_obfuscation(sock: &UdpSocket, _peer: SocketAddr, cfg: &SoildNoizeConfig) {
    if !cfg.is_enabled() {
        return;
    }
...
pub async fn send_post_handshake_junk(sock: &UdpSocket, _peer: SocketAddr, cfg: &SoildNoizeConfig) {
...
pub async fn send_keepalive_junk(sock: &UdpSocket, cfg: &SoildNoizeConfig) {
    if !cfg.is_enabled() {
        return;
    }
