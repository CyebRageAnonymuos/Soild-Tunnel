...
fn parse_address_assign(value: &[u8]) -> Result<Vec<AssignedAddress>> {
    ...
    while b.cap() > 0 {
        ...
        let addr_len = match ip_version {
            4 => 4,
            6 => 16,
            _ => return Err(SoildTunnelError::Capsule(format!("bad ip version {ip_version}"))),
        };
...
fn oct(e: octets::BufferTooShortError) -> SoildTunnelError {
    SoildTunnelError::Capsule(e.to_string())
}
