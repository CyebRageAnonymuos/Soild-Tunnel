...
use crate::soildnoize::SoildNoizeConfig;
...
pub struct WgProbe {
    ...
    pub soildnoize: SoildNoizeConfig,
    pub ports: Vec<u16>,
    pub ip: IpScan,
...
pub async fn hunt_best_wg_endpoint(probe: &WgProbe, mode: WgScanMode) -> Result<WgProbeResult> {
    hunt_wg_endpoints(probe, mode, 1)
        ...
        .into_iter()
        .next()
        .ok_or(SoildTunnelError::NoCleanEndpoint)
}
...
async fn verify_one_wg(
    ...
    let params = crate::tunnelping::WgPingParams {
        ...
        soildnoize: probe.soildnoize.clone(),
    };
...
fn build_wg_candidates(
    ...
    // >>> SOILDTUNNEL-APP-PATCH manual-wg-range
    // SoildTunnel Mobile: when the user pins their own IPv4 range(s) (Settings ->
    // endpoint mode = manual range), scan ONLY those ranges: no built-in seeds
    // and no built-in WARP prefixes. Read from SOILDTUNNEL_WG_CIDRS, else from the
    // shared SOILDTUNNEL_SCAN_CIDRS. Deliberately additive (one early return plus
    // self-contained helpers, all inside SOILDTUNNEL-APP-PATCH markers) so an
...
// >>> SOILDTUNNEL-APP-PATCH manual-wg-range
/// App-owned manual IPv4 range parser: `SOILDTUNNEL_WG_CIDRS`, else the shared
/// `SOILDTUNNEL_SCAN_CIDRS`. Upstream stopped exposing a helper for this in 1.6.0.
fn custom_wg_cidrs_v4() -> Option<Vec<String>> {
    let raw = std::env::var("SOILDTUNNEL_WG_CIDRS")
        .ok()
        .filter(|s| !s.trim().is_empty())
        .or_else(|| {
            std::env::var("SOILDTUNNEL_SCAN_CIDRS")
                .ok()
                .filter(|s| !s.trim().is_empty())
        })?;
