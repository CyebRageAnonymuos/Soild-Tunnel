...
fn describe(error: crate::error::SoildTunnelError) -> String {
    error.to_string()
}
...
#[no_mangle]
pub extern "C" fn soildtunnel_version() -> *mut c_char {
    respond(|| Ok(json!({"version": env!("CARGO_PKG_VERSION")})))
}

#[no_mangle]
pub extern "C" fn soildtunnel_string_free(raw: *mut c_char) {
    if raw.is_null() {
        return;
    }
...
#[no_mangle]
pub extern "C" fn soildtunnel_job_poll(id: u64) -> *mut c_char {
    respond(|| {
...
#[no_mangle]
pub extern "C" fn soildtunnel_job_cancel(id: u64) -> *mut c_char {
    respond(|| {
        let registry = jobs().lock();
        let job = registry
            .get(&id)
...
#[no_mangle]
pub extern "C" fn soildtunnel_job_free(id: u64) -> *mut c_char {
    respond(|| {
...
#[no_mangle]
pub extern "C" fn soildtunnel_identity_open(payload: *const c_char) -> *mut c_char {
    respond(|| {
...
#[no_mangle]
pub extern "C" fn soildtunnel_identity_summary(id: u64) -> *mut c_char {
    respond(|| {
...
#[no_mangle]
pub extern "C" fn soildtunnel_identity_free(id: u64) -> *mut c_char {
    respond(|| {
...
#[no_mangle]
pub extern "C" fn soildtunnel_scan_start(identity: u64, payload: *const c_char) -> *mut c_char {
    respond(|| {
...
#[no_mangle]
pub extern "C" fn soildtunnel_verify_start(identity: u64, payload: *const c_char) -> *mut c_char {
    respond(|| {
...
#[no_mangle]
pub extern "C" fn soildtunnel_tunnel_start(identity: u64, payload: *const c_char) -> *mut c_char {
    respond(|| {
        let payload: TunnelPayload = unsafe { read_json(payload) }?;
        let identity = identity_of(identity)?;
        let peer = socket_of(&payload.peer, "the peer address")?;
        ...
        spawn_job(move |cancel| async move {
            ...
            match api::connect(&identity, peer, &spec, &cancel).await {
                ...
                Err(crate::error::SoildTunnelError::Cancelled) => Ok(json!({"state": "stopped"})),
...
#[no_mangle]
pub extern "C" fn soildtunnel_core_start(arguments: *const c_char) -> *mut c_char {
    respond(|| {
...
#[no_mangle]
pub extern "C" fn soildtunnel_team_sign_in(payload: *const c_char) -> *mut c_char {
    respond(|| {
...
#[no_mangle]
pub extern "C" fn soildtunnel_team_code_request(payload: *const c_char) -> *mut c_char {
    respond(|| {
        let payload: TeamPayload = unsafe { read_json(payload) }?;
        let email = payload
            .email
...
#[no_mangle]
pub extern "C" fn soildtunnel_team_code_resend(session: u64) -> *mut c_char {
    respond(|| {
...
#[no_mangle]
pub extern "C" fn soildtunnel_team_code_submit(session: u64, code: *const c_char) -> *mut c_char {
    respond(|| {
...
#[no_mangle]
pub extern "C" fn soildtunnel_team_session_free(id: u64) -> *mut c_char {
    respond(|| {
...
#[no_mangle]
pub extern "C" fn soildtunnel_team_token_set(token: *const c_char) -> *mut c_char {
    respond(|| {
...
#[no_mangle]
pub extern "C" fn soildtunnel_team_token_clear() -> *mut c_char {
    respond(|| {
        let runtime = runtime().ok_or_else(|| "could not start the async runtime".to_string())?;
...
mod tests {
    ...
    fn the_version_comes_back_as_json() {
        let reply = take(soildtunnel_version());
    ...
    fn freeing_a_null_string_is_harmless() {
        soildtunnel_string_free(std::ptr::null_mut());
    }

    #[test]
    fn a_null_payload_is_reported_instead_of_crashing() {
        let reply = take(soildtunnel_identity_open(std::ptr::null()));
    ...
    fn a_payload_that_is_not_json_is_reported() {
        ...
        let reply = take(soildtunnel_identity_open(payload.as_ptr()));
        assert_eq!(reply["ok"], json!(false));
    ...
    fn an_unknown_identity_is_reported() {
        let reply = take(soildtunnel_identity_summary(999_999));
        assert_eq!(reply["ok"], json!(false));
        assert!(reply["error"]
            .as_str()
            .unwrap()
    ...
    fn an_unknown_job_is_reported() {
        let reply = take(soildtunnel_job_poll(999_999));
        assert_eq!(reply["ok"], json!(false));
    ...
    fn freeing_a_job_that_was_never_there_is_not_an_error() {
        let reply = take(soildtunnel_job_free(999_999));
        assert_eq!(reply["ok"], json!(true));
    ...
    fn a_bad_team_name_is_rejected_before_any_request_is_made() {
        ...
        let reply = take(soildtunnel_team_sign_in(payload.as_ptr()));
        assert_eq!(reply["ok"], json!(false));
    ...
    fn requesting_a_code_without_an_email_is_rejected() {
        ...
        let reply = take(soildtunnel_team_code_request(payload.as_ptr()));
        assert_eq!(reply["ok"], json!(false));
    ...
    fn a_token_that_is_not_a_jwt_is_refused() {
        ...
        let reply = take(soildtunnel_team_token_set(token.as_ptr()));
        assert_eq!(reply["ok"], json!(false));
    ...
    fn a_job_runs_to_completion_and_can_be_polled() {
        ...
        for _ in 0..200 {
            let polled = take(soildtunnel_job_poll(id));
    ...
    fn a_cancelled_job_reports_that_it_was_cancelled() {
        ...
        take(soildtunnel_job_cancel(id));
    ...
    fn the_core_refuses_an_argument_list_that_is_not_a_json_array() {
        ...
        let reply = take(soildtunnel_core_start(payload.as_ptr()));
        assert_eq!(reply["ok"], json!(false));
    ...
    fn an_unparsable_socks_address_is_reported() {
        ...
        let reply = take(soildtunnel_tunnel_start(1, payload.as_ptr()));
        assert_eq!(reply["ok"], json!(false));
    }
    ...
    fn an_unparsable_peer_is_reported() {
        ...
        let reply = take(soildtunnel_verify_start(1, payload.as_ptr()));
        assert_eq!(reply["ok"], json!(false));
    }
}
