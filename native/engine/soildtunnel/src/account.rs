...
pub fn generate_masque_keypair() -> Result<MasqueKeyPair> {
    ...
    let ec = EcKey::generate(&group).map_err(|e| SoildTunnelError::Tls(e.to_string()))?;
    let pkey = PKey::from_ec_key(ec).map_err(|e| SoildTunnelError::Tls(e.to_string()))?;
    ...
    let mut builder = X509Builder::new().map_err(|e| SoildTunnelError::Tls(e.to_string()))?;
    builder
        .set_version(2)
        .map_err(|e| SoildTunnelError::Tls(e.to_string()))?;
    ...
    let not_before = Asn1Time::days_from_now(0).map_err(|e| SoildTunnelError::Tls(e.to_string()))?;
...
fn http_client() -> Result<reqwest::Client> {
    ...
    builder.build().map_err(|e| SoildTunnelError::Api(e.to_string()))
}
...
async fn fallback_call(
    ...
    if matches!(response.status, 401 | 404 | 410) {
        return Err(SoildTunnelError::IdentityRefused(described));
    }
    Err(SoildTunnelError::Api(described))
}
...
{
    let mut last_error = SoildTunnelError::Api(format!("{label}: no attempt was made"));
    ...
    for attempt in 0..API_ATTEMPTS {
        ...
        let body = response
            .text()
            .await
            .map_err(|e| SoildTunnelError::Api(format!("{label}: {e}")))?;

        if status.is_success() {
            return serde_json::from_str::<AccountData>(&body)
                .map_err(|e| SoildTunnelError::Api(format!("{label} decode: {e}; body={body}")));
        }

        let described = format!("{label}: {}", describe_rejection(status, &body));
        last_error = if refuses_identity(status) {
            SoildTunnelError::IdentityRefused(described)
        } else {
            SoildTunnelError::Api(described)
        };
...
pub async fn register(model: &str, locale: &str, jwt: Option<&str>) -> Result<(AccountData, [u8; 32])> {
    ...
    let encoded =
        serde_json::to_vec(&body).map_err(|e| SoildTunnelError::Api(format!("encode: {e}")))?;
...
pub async fn enroll_key(
    ...
    match direct {
        ...
        Err(primary) => {
            ...
            {
                Ok(account) => Ok(account),
                Err(secondary) => Err(SoildTunnelError::Api(format!(
...
fn extract_wg_peer(reg: &AccountData) -> Result<[u8; 32]> {
    if reg.config.peers.is_empty() {
        return Err(SoildTunnelError::Api("no peers in registration response".into()));
    }
    ...
    let decoded = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, peer_b64)
        .map_err(|e| SoildTunnelError::Api(format!("decode peer pubkey: {e}")))?;
    if decoded.len() != 32 {
        return Err(SoildTunnelError::Api("invalid peer pubkey length".into()));
    }
    let mut arr = [0u8; 32];
...
fn finish_provision(reg: AccountData, wg_private: [u8; 32]) -> Result<Identity> {
    if reg.token.is_empty() {
        return Err(SoildTunnelError::Api("registration returned empty token".into()));
    }
