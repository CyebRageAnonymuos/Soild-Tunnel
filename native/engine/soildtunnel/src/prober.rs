...
fn build_candidates(st: &Strategy, ports: &[u16], ip: IpScan) -> Vec<(IpAddr, u16)> {
    ...
    // >>> SOILDTUNNEL-APP-PATCH manual-masque-range
    // SoildTunnel Mobile: same manual-range override as wg_prober.rs, for the
    // MASQUE/gool scanner. Reads SOILDTUNNEL_MASQUE_CIDRS, else SOILDTUNNEL_SCAN_CIDRS.
...
/// `SOILDTUNNEL_MASQUE_CIDRS`, else the shared `SOILDTUNNEL_SCAN_CIDRS`.
fn custom_masque_cidrs_v4() -> Option<Vec<String>> {
    let raw = std::env::var("SOILDTUNNEL_MASQUE_CIDRS")
        .ok()
...
mod tests {
    ...
    async fn report_which_masque_ranges_answer_on_this_network() {
        ...
        let timeout = Duration::from_millis(
            std::env::var("SOILDTUNNEL_PROBE_TIMEOUT_MS")
                .ok()
        ...
        let attempts = std::env::var("SOILDTUNNEL_PROBE_ATTEMPTS")
            .ok()
            .and_then(|raw| raw.trim().parse::<u32>().ok())
            .filter(|n| *n > 0)
            .unwrap_or(2);

        let ports: Vec<u16> = match std::env::var("SOILDTUNNEL_PROBE_PORTS") {
            Ok(raw) => raw
                .split(',')
        ...
        println!("  override: SOILDTUNNEL_PROBE_TIMEOUT_MS, SOILDTUNNEL_PROBE_ATTEMPTS, SOILDTUNNEL_PROBE_PORTS");
        println!();
