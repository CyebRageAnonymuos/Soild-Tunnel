...
fn data_check_enabled() -> bool {
    std::env::var("SOILDTUNNEL_MASQUE_NO_DATA_CHECK").is_err()
}

fn validation_timeout() -> Duration {
    let secs = std::env::var("SOILDTUNNEL_MASQUE_VALIDATE_SECS")
        .ok()
...
fn h2_keepalive_interval() -> Duration {
    let secs = std::env::var("SOILDTUNNEL_MASQUE_H2_KEEPALIVE_SECS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .filter(|&v| v > 0)
        .unwrap_or(15);
...
fn h2_keepalive_timeout() -> Duration {
    let secs = std::env::var("SOILDTUNNEL_MASQUE_H2_KEEPALIVE_TIMEOUT_SECS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .filter(|&v| v > 0)
        .unwrap_or(20);
...
pub fn enabled() -> bool {
    match std::env::var("SOILDTUNNEL_MASQUE_HTTP2") {
        Ok(v) => {
...
pub fn h2_peer(quic_peer: SocketAddr) -> SocketAddr {
    if let Ok(v) = std::env::var("SOILDTUNNEL_MASQUE_H2_PEER") {
...
fn build_tls(cfg: &H2TunnelConfig) -> Result<boring::ssl::ConnectConfiguration> {
    let mut builder =
        SslConnector::builder(SslMethod::tls()).map_err(|e| SoildTunnelError::Tls(e.to_string()))?;

    builder
        .set_min_proto_version(Some(SslVersion::TLS1_2))
        .map_err(|e| SoildTunnelError::Tls(e.to_string()))?;
    builder
    ...
    let groups = std::env::var("SOILDTUNNEL_TLS_GROUPS").ok();
    let groups = groups
        .as_deref()
        .map(str::trim)
        .filter(|s| !s.is_empty())
    ...
    let cert = X509::from_pem(&cfg.cert_pem).map_err(|e| SoildTunnelError::Tls(e.to_string()))?;
    let key =
        PKey::private_key_from_pem(&cfg.key_pem).map_err(|e| SoildTunnelError::Tls(e.to_string()))?;
    builder
...
fn build_connect_request(cfg: &H2TunnelConfig) -> Result<http::Request<()>> {
    ...
    http::Request::builder()
        ...
        .header("user-agent", "")
        .body(())
        .map_err(|e| SoildTunnelError::Masque(format!("build request: {e}")))
}
...
pub async fn dial(peer: std::net::SocketAddr) -> Result<TcpStream> {
    match crate::upstream::configured() {
        ...
        None => TcpStream::connect(peer).await.map_err(SoildTunnelError::Io),
    }
}
...
pub async fn verify_h2(cfg: &H2TunnelConfig, timeout: Duration) -> Result<Duration> {
    ...
    let attempt = async {
        ...
        let (h2, connection) = h2::client::handshake(tls)
            .await
            .map_err(|e| SoildTunnelError::Masque(format!("h2 handshake: {e}")))?;
        ...
        let mut h2 = h2
            .ready()
            .await
            .map_err(|e| SoildTunnelError::Masque(format!("h2 ready: {e}")))?;
        let req = build_connect_request(cfg)?;
        let (resp_fut, mut send_stream) = h2
            .send_request(req, false)
            .map_err(|e| SoildTunnelError::Masque(format!("send_request: {e}")))?;
        let response = resp_fut
            .await
            .map_err(|e| SoildTunnelError::Masque(format!("await response: {e}")))?;
        let status = response.status();
        if !status.is_success() {
            driver.abort();
            return Err(SoildTunnelError::Masque(format!(
        ...
        loop {
            match futures::future::poll_fn(|cx| recv_body.poll_data(cx)).await {
                ...
                Some(Err(e)) => {
                    driver.abort();
                    return Err(SoildTunnelError::Masque(format!("h2 body: {e}")));
                }
                None => {
                    driver.abort();
                    return Err(SoildTunnelError::Masque("h2 stream closed before data".into()));
                }
            }
        }
    };
    ...
    match tokio::time::timeout(timeout, attempt).await {
        ...
        Ok(Err(e)) => Err(e),
        Err(_) => Err(SoildTunnelError::Other("h2 verify timeout".into())),
    }
}
...
pub async fn run(
    ...
    let mut ping_pong = connection.ping_pong().ok_or_else(|| {
        SoildTunnelError::Masque("h2 connection does not support ping".into())
    })?;
    ...
    loop {
        if data_check && !ready_fired {
            if let Some(dl) = validate_deadline {
                if Instant::now() >= dl {
                    ...
                    return Err(SoildTunnelError::Masque(
        ...
        if let Some(dl) = pong_deadline {
            if Instant::now() >= dl {
                ...
                let _ = send_stream.send_data(Bytes::new(), true);
                return Err(SoildTunnelError::Masque("h2 keepalive timeout".into()));
            }
        }
            ...
            pong = std::future::poll_fn(|cx| ping_pong.poll_pong(cx)), if awaiting_pong => {
                match pong {
                    ...
                    Err(e) => {
                        ...
                        let _ = send_stream.send_data(Bytes::new(), true);
                        return Err(SoildTunnelError::Masque(format!("h2 keepalive: {e}")));
                    }
                }
            }
...
async fn send_capsule(send: &mut h2::SendStream<Bytes>, data: Bytes) -> Result<()> {
    ...
    while send.capacity() < len {
        match futures::future::poll_fn(|cx| send.poll_capacity(cx)).await {
            Some(Ok(_)) => {}
            Some(Err(e)) => return Err(SoildTunnelError::Masque(format!("h2 capacity: {e}"))),
            None => return Err(SoildTunnelError::Masque("h2 stream closed".into())),
        }
    }

    send.send_data(data, false)
        .map_err(|e| SoildTunnelError::Masque(format!("h2 send_data: {e}")))?;
    Ok(())
}
