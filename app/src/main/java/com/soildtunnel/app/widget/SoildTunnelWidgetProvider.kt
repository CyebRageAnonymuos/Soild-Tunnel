package com.soildtunnel.app.widget
...
/**
 * Home-screen widget, ported from the merged SoildTunnelWidgetProvider and adapted
 * to SoildTunnel Mobile's [SoildTunnelController] / [ProfileStore] architecture.
 *
 ...
 */
class SoildTunnelWidgetProvider : AppWidgetProvider() {

    companion object {
        const val ACTION_TOGGLE = "com.soildtunnel.app.WIDGET_TOGGLE"

        /** Repaints every placed widget; called on each connection-state change. */
        fun updateAllWidgets(context: Context) {
            val manager = AppWidgetManager.getInstance(context) ?: return
            val component = ComponentName(context, SoildTunnelWidgetProvider::class.java)
            val ids = manager.getAppWidgetIds(component)
            if (ids.isEmpty()) return
            val state = SoildTunnelController.state.value
        ...
        private fun paint(
            ...
            val views = RemoteViews(context.packageName, R.layout.soildtunnel_widget)
            ...
            val toggle = PendingIntent.getBroadcast(
                context,
                0,
                Intent(context, SoildTunnelWidgetProvider::class.java).setAction(ACTION_TOGGLE),
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (state.isConnected || state.isBusy) {
            SoildTunnelController.disconnect(context)
            return
        }
        ...
        Thread {
            try {
                ...
                SoildTunnelController.connect(context.applicationContext, profile)
            } finally {
                pending.finish()
            }
        }.start()
