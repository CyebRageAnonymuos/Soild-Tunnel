 ...
 * process that builds the TUN + engine config and the UI/diagnostics layer
 * (which talks to the local SOCKS5 proxy to probe connectivity and geolocation).
 *
 * IMPORTANT: [TUN_IPV4] MUST be written into BOTH the TUN interface address AND
 * the hev-socks5-tunnel `tunnel.ipv4` field. Battle-tested clients always set
...
object TunnelConfig {
     ...
     * [com.soildtunnel.desktop.model.ConnectionProfile.mtu]; this constant is only
